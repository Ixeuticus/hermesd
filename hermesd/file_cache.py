...
class LastGoodFileCache:
    ...
    def read_json_mapping(self, path: Path) -> dict[str, Any]:
        with self._lock:
            ...
            if self._json_mtimes.get(key) == mtime and key in self._json_values:
    ...
    def read_json_list(self, path: Path) -> list[dict[str, Any]]:
        with self._lock:
            ...
            if self._json_list_mtimes.get(key) == mtime and key in self._json_lists:
            ...
            if not isinstance(value, list) or not all(isinstance(entry, dict) for entry in value):
                self._json_list_bad_mtimes[key] = mtime
                return self._json_lists.get(key, [])
            self._json_list_mtimes[key] = mtime
    ...
    def read_yaml_mapping(self, path: Path) -> dict[str, Any]:
        with self._lock:
            ...
            if self._yaml_mtimes.get(key) == mtime and key in self._yaml_values:
